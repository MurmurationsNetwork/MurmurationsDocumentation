...
We start with defining the lists of schemas we want our data to conform to using the `linked_schemas` field. Next, we add all of the fields required by both of those schemas. We host this data ideally on our own servers, or using a third party hosting service (like [MPG](https://test-tools.murmurations.network/profile-generator)). Finally, we submit the URL where this data is hosted to the Murmurations index so it is recorded and [can be searched for by anyone](https://test-tools.murmurations.network/index-explorer?schema=organizations_schema-v1.0.0) who is interested in finding organizations who define themselves according to Alice's or Bob's organizations schemas.
